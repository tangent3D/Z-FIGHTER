...
__Note for advanced users__: The aforementioned DIP switch is likewise connected to the SPLD allowing for custom memory maps per ROM. Using combinational logic with ROM A18-A16 and address bus lines A15-A13 as SPLD inputs, custom memory maps can be defined in chunks of 8K, e.g. 56K ROM/8K RAM.[^1] Care should be taken that ROMs are compiled to use the intended memory map.
