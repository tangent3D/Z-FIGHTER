...
A 3-position DIP switch (connected to ROM A18, A17, A16) will manually page the 512K ROM, affording the user eight bootable ROMs to be installed on the system. All ROMS should be placed at base addresses of 0000h, 10000h, ..., 70000h.

__Note for advanced users:__ The aforementioned DIP switch is likewise connected to the SPLD. Using combinational logic with ROM A18-A16 and Z80 address bus lines A15-A13, custom memory maps per selected ROM page can be defined in chunks of 8K, e.g. 56K ROM/8K RAM.[^1] Care should be taken that ROMs are compiled to use the intended memory map.
